    ...
    for l in dsndiag:
        ...
        if not any(x.lower() in str(l).lower() for x in not_bounces):
            ...
            if not _email in all_bounces:
                if flag == False:
                    l_bouncecatg = str(bounceCat[i]).lower()
                    for bc in bouncecatg_pattern:
                        if bc.lower() in l_bouncecatg:
                            all_bounces[_email] = ["Soft",dsnstatus[i],l,bounceCat[i]]
                            flag = True
                            break
                if flag == False:
                    ...
                    for bp in bounce_patterns:
                        if bp.lower() in l_dsndiag:
                            ...
                            if check_false_positive(l_dsndiag) == False:
                                all_bounces[_email] = ["Hard",dsnstatus[i],l,bounceCat[i]]
                                flag = True
                            break
                if flag == False:
                ...
                if flag == False:
                    ...
                    if len(dlvdestip) < 2:
                        all_bounces[_email] = ["Hard",dsnstatus[i],dsndiag[i],bounceCat[i]]
                        flag = True
                        break
...
if len(all_bounces) > 0:
    ...
    for r in all_bounces:
        ...
        bounce_category = all_bounces[r][3]
        for rp in replace_regex:
            bounce_reason = sub( rp[0], rp[1], str(bounce_reason) )
        outputf.write(f""""{r}","{all_bounces[r][0]}","{all_bounces[r][1]}","{bounce_reason}","{bounce_category}"\n""")
    outputf.close()
